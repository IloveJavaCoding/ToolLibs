package com.nepalese.toollibs.OverWriteClass.Drawable;
...
public class Triangle {
    ...
    public Triangle(Context context, Random random, int w, int h, int r) {
        ...
        tx = w / 2 + r * 2 / 3;
        ty = h / 2;
        sx = ConvertUtil.dip2px(context, random.nextFloat() + 2.0f);
        sy = ConvertUtil.dip2px(context, random.nextFloat() + 1.0f);
    ...
    public void draw(Canvas canvas, Paint paint, Matrix matrix, Path p) {
        ...
        tx = tx + (ConvertUtil.dip2px(mContext, deltaTime * 2.0f / 1000));
        degrees = degrees + (ConvertUtil.dip2px(mContext, deltaTime * 0.3f / 1000));
        if (alpha > 0) {
            alpha = alpha - (ConvertUtil.dip2px(mContext, deltaTime * 1.0f / 1000));
        }
        if (alpha < 0) {
            alpha = 0;
        }
