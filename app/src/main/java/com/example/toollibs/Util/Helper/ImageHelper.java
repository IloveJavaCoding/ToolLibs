...
public class ImageHelper {
    ...
    public static MyImageView createMyImageView(Context context){
        MyImageView myImageView = new MyImageView(context);
        //默认淡入淡出
        setAnimation(myImageView, ANIM_TYPE_FADE_IN, ANIM_TYPE_FADE_OUT);
        return myImageView;
    }

    public static void setAnimation(MyImageView imageView, int animIn, int animOut){
        if(imageView==null){
            return;
        }
    ...
    public static void setImgUri(MyImageView imageView, List<String> list, int delayIndex){
        if(imageView==null){
            return;
        }
